...
import { parseDOCX, prepareDocument, compileDocument } from "../src";

const __dirname = dirname(fileURLToPath(import.meta.url));

const docxDir = join(__dirname, "docx"); // input: DOCX files placed here manually
const tempDir = join(__dirname, ".temp");
const jsonDir = join(tempDir, "json");
const docxOutDir = join(tempDir, "docx");

// Create/clean output directories
for (const dir of [jsonDir, docxOutDir]) {
  if (existsSync(dir)) {
...
console.log(`⏳ Testing ${docxFiles.length} files: DOCX → JSON → (prepare → compile → generate)\n`);
...
for (const docxFile of docxFiles) {
  try {
    ...
    // Step 2: Tiptap JSON → DOCX — split into prepare/compile/generate to
    ...
    results.push({
      file: docxFile,
      success: true,
      timings: { parse: tParse, prepare: tPrepare, compile: tCompile, generate: tGenerate },
    });
    console.log(
      `  ✅ ${docxFile}  parse ${tParse.toFixed(1)} · prep ${tPrepare.toFixed(1)} · comp ${tCompile.toFixed(1)} · gen ${tGenerate.toFixed(1)} (ms)`,
    );
  } catch (error) {
...
const timed = results
  ...
  .map((r) => ({
    file: r.file,
    ...r.timings,
    total: r.timings.parse + r.timings.prepare + r.timings.compile + r.timings.generate,
  }))
...
if (timed.length) {
  console.log(`\n⏱  Timings (ms):`);
  console.log(
    `  ${"file".padEnd(34)} ${"parse".padStart(7)} ${"prep".padStart(7)} ${"comp".padStart(7)} ${"gen".padStart(8)} ${"total".padStart(8)}`,
  );
  for (const t of timed) {
    console.log(
      `  ${t.file.slice(0, 34).padEnd(34)} ${t.parse.toFixed(1).padStart(7)} ${t.prepare.toFixed(1).padStart(7)} ${t.compile.toFixed(1).padStart(7)} ${t.generate.toFixed(1).padStart(8)} ${t.total.toFixed(1).padStart(8)}`,
    );
  }
  const sum = timed.reduce(
    ...
    { parse: 0, prepare: 0, compile: 0, generate: 0, total: 0 },
  );
  console.log(
    `  ${"".padEnd(34)} ${"-".repeat(7)} ${"-".repeat(7)} ${"-".repeat(7)} ${"-".repeat(8)} ${"-".repeat(8)}`,
  );
  console.log(
    `  ${`total (${timed.length})`.padEnd(34)} ${sum.parse.toFixed(1).padStart(7)} ${sum.prepare.toFixed(1).padStart(7)} ${sum.compile.toFixed(1).padStart(7)} ${sum.generate.toFixed(1).padStart(8)} ${sum.total.toFixed(1).padStart(8)}`,
  );
}
