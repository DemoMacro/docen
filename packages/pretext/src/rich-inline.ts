...
export function prepareRichInline(items: RichInlineItem[]): PreparedRichInline {
  ...
  for (let index = 0; index < items.length; index++) {
    ...
    if (trimmedText.length === 0) {
      ...
      // survives whitespace trimming as a zero-text atomic item. break:'never'
      // keeps it unbreakable; the empty prepared carries no glyphs and
      // materializes as ''.
      const extraWidth = item.extraWidth ?? 0;
      if (extraWidth > 0) {
        const atomicItem: PreparedRichInlineItem = {
          break: "never",
          endGraphemeIndex: 0,
          endSegmentIndex: 0,
          extraWidth,
          gapBefore: pendingGapWidth > 0 ? pendingGapWidth : 0,
          naturalWidth: 0,
          prepared: prepareWithSegments("", item.font),
          sourceItemIndex: index,
        };
        preparedItems.push(atomicItem);
        itemsBySourceItemIndex[index] = atomicItem;
        pendingGapWidth = 0;
        continue;
      }
      if (COLLAPSIBLE_BOUNDARY_RE.test(item.text) && pendingGapWidth === 0) {
        pendingGapWidth = getCollapsedSpaceWidth(
          item.font,
          letterSpacing,
...
export function layoutNextRichInlineLineRange(
  ...
  const width = stepRichInlineLine(
    flow,
    maxWidth,
    end,
