...
describe("table cell property commands", () => {
  ...
  it("sort-table is disabled when table contains merged cells", () => {
    const editor = build();
    editor.commands["insert-table"]();
    caretInCell(editor, 0, 0);
    // Mark a cell merged
    ...
    editor.state.doc.descendants((node, pos) => {
      if (node.type.name === "tableCell") {
        tr.setNodeMarkup(pos, undefined, { ...node.attrs, columnSpan: 2 });
        return false;
      }
      return true;
    });
    editor.view.dispatch(tr);
    expect(editor.commands["sort-table"]()).toBe(false);
  });
  ...
  it("table-formula replaces active selection inside cell preserving surrounding paragraphs", () => {
    const editor = build();
    editor.commands["insert-table"]();
    caretInCell(editor, 1, 0);

    // Insert text into cell (1, 0)
    const { state } = editor;
    const { from } = state.selection;
    editor.view.dispatch(
      state.tr.insert(from, [
        state.schema.nodes.paragraph.create(null, state.schema.text("First line")),
      ]),
    );

    caretInCell(editor, 1, 0);
    expect(editor.commands["table-formula"]("=50+50")).toBe(true);
    const cell = tablesOf(editor)[0]!.child(1).child(0);
    expect(cell.childCount).toBeGreaterThanOrEqual(2);
  });
});
