...
template.innerHTML = `
  <style>
    ...
    [part="start"] { display: flex; align-items: center; gap: 4px; flex: 0 0 auto; }
    [part="search"] { flex: 1 1 auto; display: flex; justify-content: center; }
    [part="end"] {
      display: flex;
      align-items: center;
      gap: 4px;
    ...
    ::slotted([slot="search"]) { width: 100%; max-width: 480px; }
  </style>
...
/**
 * `<docen-app-header>` — the workspace's global header (Office title bar):
 ...
 */
class DocenAppHeader extends HTMLElement {
...
customElements.define("docen-app-header", DocenAppHeader);

export default DocenAppHeader;
