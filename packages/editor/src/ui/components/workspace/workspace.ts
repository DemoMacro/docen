...
template.innerHTML = `
  <style>
    ...
    .rb-shell-ribbon { flex: 0 0 auto; position: relative; }
    .rb-shell-content {
      flex: 1 1 auto;
      min-height: 0;
      display: flex;
    }
    .rb-shell-content .rb-canvas { flex: 1 1 auto; min-width: 0; display: flex; }
    .rb-shell-content .rb-aside { flex: 0 0 auto; min-width: 0; overflow: hidden; }
    .rb-shell-status {
      flex: 0 0 auto;
      display: flex;
      align-items: center;
    ...
    :host([data-fullscreen]) .rb-shell-status { display: none; }
    /* Print: hide everything but the canvas (the printed page). */
    @media print {
      :host { display: block; height: auto; background: #fff; }
      .rb-shell-header,
      .rb-shell-ribbon,
      .rb-shell-status,
      .rb-aside { display: none !important; }
      .rb-shell-content { display: block; }
 ...
 */
class DocenWorkspace extends HTMLElement {
...
customElements.define("docen-workspace", DocenWorkspace);
