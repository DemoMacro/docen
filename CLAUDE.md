...
`doc > page+`, each `page` a **fixed-height box**; a ProseMirror `appendTransaction` **physically reflows** overflow to the next page. This is the only route yielding Office-style fixed pages with **edit == render** — a single contenteditable, no separate painter, no selection/cursor coordinate mapping. The page node is editing-time only and never enters DOCX. Implementation rules: CONTRIBUTING.md → Pagination Conventions.
...
packages/editor/src/ — multi-editor host + add-ins
  ...
  document/       <docen-document>: index.ts · addin.ts (default add-in) · ribbon.ts · commands.ts · components/ (format-pane/outline — document-specific UI) · pagination/ · extensions/
